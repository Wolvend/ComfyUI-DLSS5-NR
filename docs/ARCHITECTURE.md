...
The Neural Rendering path is D3D12, while v0.3.1 uses the NVOF **D3D11** driver entry point on a private D3D11 device created from the same `IDXGIAdapter1`.
